...
export default defineComponent({
    ...
    setup() {
        ...
        const fetchVideos = async () => {
            try {
                const res = await classTeachersApi.getAlbumsByTeacherId(user.id); // 获取教师的合集
                const albumIds = res.data.map((album: any) => album.classId); // 提取合集 ID 列表

                const response = await videoApi.getAllVideos();
                videos.value = response.data.filter((video: any) => albumIds.includes(video.albumId));
        ...
        const handleSearch = async () => {
            ...
            try {
                ...
                // 同样过滤教师自己的合集
                videos.value = response.data.filter((video: any) => video.albumId === teacherId);
                currentPage.value = 1;
            } catch (error) {
